...
Ce projet a été réalisé par Adrien Magneron (`amagnero`), Lucas Mabille (`lumabill`), Haotian Zhu (`zhuhaoti`) et Eugène Valty (`valtyeug`) dans le cadre de l'UV SR05 (Algorithmes et systèmes répartis).
Il permet de mettre en oeuvre la duplication et répartition de données bancaires sur différents sites, en s'assurant que les données soit cohérentes entre chaque sites, en utilisant un algorithme de **file repartie**. Il permet aussi de prendre un instantané global du système en utilisant cette fois ci un algorithme de **snapshot**.
...
Le programme peut être lancé via le script `launch.sh`, en précisant le nombre de sites voulus en argument.

## Description générale des sites

### Généralites

Chaque site est composé de deux élements : `BAS` et `NET`. `BAS` représente l'application de base, ayant accès aux données (la **"section critique"**). Les messages destinés à `BAS` sont pré-traités par `NET`, qui gère si oui ou non `BAS` a accès à ces données en utilisant l'algorithme de **file repartie** *(cf. partie correspondante)*. `NET` communique avec son `BAS` et les autres `NET`.
...
`APP` représente une application, lisant constamment dans l'entrée standard, et renvoie chaque réponse à une fonction de réception, à implémenter selon le type d'application.
...
`NET` gère les messages reçus pour et par `BAS` sur le réseau, et les traite selon le type de messages reçus, et utilise l'algorithme de **file repartie** pour s'assurer qu'aucun accès concurrent aux données soient possibles.
...
La fonction de réception héritée de `APP` et implémentée par `NET` implémente les gardes décrites dans l'algorithme de file d'attente et utilise des **estampilles** *(horloges logiques)*.
...
Les snapshot sont initiées par `BAS`, puis gérées par `NET` qui compte le nombre de sites restant. Chaque site ayant effectué sa capture devient rouge, s'assurant qu'une seule capture ne soit faite et que les messages ne circulent pas indéfiniement.
...
***1.** Envoi d'une demande de section critique par `BAS`*
...
Exécution imagée de l'algorithme, pour les mêmes raisons que précédemment.
...
***1.** Envoi d'une demande de snapshot par `BAS`*
...
***2.** Réception de la requête par `NET`, passage en rouge et déclaration en tant qu'initiateur et retransmission aux autres `NET`*
...
***3.** Réception de la requête par les autres `NET` et retransmission a leur `BAS`*
...
***4.** Renvoi de la capture par `BAS` a son `NET`*
...
***5.** Renvoi de la capture au `NET` initiateur*
...
***7.** Le `BAS` initiateur écrit la capture dans un fichier log*

## Architecture

Le projet contient plusieurs fichiers `python`, chacun ayant une fonction particulère : 
...
Gère la création de message, plus précisement le **payload**, via un dictionnaire de couple `clé`/`valeur`, leur création via un `string` au format `AirPlug` ou via un dictionnaire déjà instancié et leur passage au format texte `AirPlug`.
...
Comme décrit précedemment, possède un dictionnaire de couple `clé`/`valeur` (les paramètres), un constructeur permettant la création à partir d'un dictionnaire pré-rempli (ou non) et une liste de paramètres nécessaires (ou non), une méthode d'envoi, une de réception **abstraite** et quelques méthodes auxiliaires.

### `bas.py`

Hérite de `App`, réimplémente le constructeur pour compléter les paramètres, ainsi que la méthode de réception pour renvoyer aux différentes fonctions selon le type de requête. Une classe `BasWindow` est aussi dans le module et gère l'interface graphique de `BAS`.

### `net.py`

Hérite de `App`, réimplémente le constructeur pour compléter les paramètres, ainsi que la méhode de réception pour renvoyer aux différentes fonctions selon le type de requête. Comme décrit précedemment, implémente l'algorithme de la file d'attente en instanciant les paramètres nécessaires à sa création.
