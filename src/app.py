...
class App:
    ...
    def send(self, msg: Message = None, who: str = None) -> None:
        ...
        msg_str = "{}{}{}".format(header, self.__delimiter__(),
                                  msg.format())
        printerr("[{}] Message sent ! ({})".format(
            self.__appname__(), msg_str))
        sys.stdout.write("{}\n".format(msg_str))
        sys.stdout.flush()

    def parse(self, input: str = None) -> dict:
        # printerr("[{}] parsing '{}'...'".format(self.__appname__(), input))
        L = input.rstrip("\n").split(self.__delimiter__())[1:]
        # must have 3 fields
        if len(L) != 3:
